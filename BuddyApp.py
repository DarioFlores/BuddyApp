...
class MiVentana(QMainWindow):
    ...
    def sacar_proceso_memoria_principal(self):
        ...
        text = self.memoriaPrincipal.sacar(proceso)
        self.plainTextEditAcciones.appendPlainText(text)
...
#LISTOOOO
    ...
    def sacar(self, proceso):
        ...
        bindirCom = bin(int(particion.direccionComienzo))
        binCabal = bin(2 ** int(particion.cabal))
        dircompa = int(particion.direccionComienzo) ^ (2 ** int(particion.cabal))
        binDirCompa = bin(int(dircompa))
        text = text + "\tDir. Com =   " + str(bindirCom) + " = " + str(particion.direccionComienzo) + "\n" \
                      "\tKval =       " + str(binCabal) + " = 2^" + str(particion.cabal) + "\n" \
                      "\t              ----------------------------\n" \
                      "\tDir. Compa = " + str(binDirCompa) + " = " + str(dircompa) + "\n"
        return text + str(self.compactar(padre))

    def compactar(self, padre):
        text = ""
        if padre is None:                   #ES EL NODO RAIZ
            text = text + "-----------NO SE PUEDE COMPACTAR---------\n"
            ...
            if padre.hijoIzquierdo.libre() and padre.hijoDerecho.libre():
                ...
                bindirCom = bin(int(padre.direccionComienzo))
                binCabal = bin(2 ** int(padre.cabal))
                dircompa = int(padre.direccionComienzo) ^ (2 ** int(padre.cabal))
                binDirCompa = bin(int(dircompa))
                text = text + "\tDir. Com =   " + str(bindirCom) + " = " + str(padre.direccionComienzo) + "\n" \
                              "\tKval =       " + str(binCabal) + " = 2^" + str(padre.cabal) + "\n" \
                              "\t              ----------------------------\n" \
                              "\tDir. Compa = " + str(binDirCompa) + " = " + str(dircompa) + "\n"

                text = text + str(self.compactar(padre.padre))
            else:
                text = text + "-----------NO SE PUEDE COMPACTAR---------\n"
        return text
